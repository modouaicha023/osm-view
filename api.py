...
app = Flask(__name__)
CORS(app)  # Activer CORS pour l'application mobile
    ...
    try:
        ...
        for point in points:
            ...
            if not validate_time_window(point['arrival_time']):
                point['arrival_time'] = "08:00"  # Réinitialiser à 8h si invalide
...
def optimize_routes():
    try:
        ...
        num_drivers = min(max(1, config.get('num_drivers', 3)), 5)  # Entre 1 et 5 chauffeurs
        capacity = min(max(4, config.get('capacity_per_driver', 8)), 12)  # Entre 4 et 12 passagers
        max_distance = min(max(5, config.get('max_distance_km', 15)), 20)  # Entre 5 et 20 km
...
def get_driver_route(driver_id):
    try:
        ...
        driver_routes = routes_df[routes_df['driver_id'] == driver_id].to_dict('records')
        
        if not driver_routes:
            abort(404, description=f"Aucun itinéraire trouvé pour le chauffeur {driver_id}")
